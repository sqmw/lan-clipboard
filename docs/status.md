...
- 当前发送模型是对发现到的共享域成员广播；没有“选择共享域内某一个成员发送文件”的目标选择器、单 peer IPC 或稳定目标身份路由。
- 文件剪贴板回归尚未形成完整的 macOS Finder / Windows Explorer 验收证据：复制文件后应能在桌面或文件夹目标中直接粘贴并创建文件；若该路径失败，属于待确认的跨平台剪贴板写回缺陷。
- 文本输入框不是文件剪贴板目标。把文件粘贴到文本输入框不承诺插入文件路径或文件内容；该场景应记录为不支持的目标语义，不与 Finder / Explorer 文件粘贴混为同一个问题。
...
3. 支持选择共享域内的单个目标成员发送文件。
4. 完成 Finder / Explorer 文件粘贴互操作矩阵，并明确文本输入框的“不支持文件粘贴”行为。
5. 失败原因进一步面向用户细分。
6. 发布前真实双端剪贴板互操作矩阵。
