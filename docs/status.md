...
从 `1.x` 升级时，所有设备必须一起升级并重新配对。应用会在覆盖旧设置前保留 legacy/invalid 原始备份；不会让 `v4` 降级连接旧协议。
...
- 同步与加密是 `v4` 不变量；UI 用只读状态徽标展示“已启用”，不再伪装成可操作的关闭开关。
...
| portable payload | `8MiB` |
| file raw frame | `16MiB` |
| 文件读缓冲 | `1MiB` |
...
- portable 文本/图片/富文本为单帧内存模型，硬上限 8MiB；更大的内容需要未来流式协议。
...
1. 逐设备密钥、成员撤销与前向保密。
2. portable 大 payload 流式化与纯网络 benchmark。
3. 失败原因进一步面向用户细分。
4. 发布前真实双端剪贴板互操作矩阵。
