...
生产时间与并发值保留在代码常量中；测试通过 loopback、小 payload、直接 codec 和内部可注入期限覆盖状态转换，不修改生产值。关键生产值包括：握手绝对总期限/连接超时 `2s`、帧 idle `30s` 与按大小计算的 `8s..120s` 总期限、文件接收 idle `30s` 与 `31s..30min` 总期限、UDP `500ms`、后台发现 `3s`、成员发现 TTL `30s`、后台/手动单次发现预算 `900/2200ms`、发送 peer 并发 `8`、入站连接 `16`/单 IP `4`、同时文件接收 `2`。
