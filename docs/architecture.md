...
当前文件发送已经改为更细粒度的分片数据面：bulk worker 以约 `1MB` chunk 推进文件流，并在 chunk 边界检查高优先级发送任务，必要时主动短暂让出。
当前 bulk 数据面还增加了“发送前准备层”：同一条文件复制事件只会在本机临时目录里打一次 `tar` 归档，之后由多个 peer 复用同一份已准备好的归档文件，避免广播时对每个 peer 重复扫描磁盘和重复打包。
当前 TCP 连接初始化统一走传输调优入口：发送端和接收端都会设置 `nodelay`、动态超时和约 `4MB` 的收发缓冲，优先把局域网场景下的吞吐瓶颈留给磁盘与系统剪贴板，而不是默认 socket 缓冲。
