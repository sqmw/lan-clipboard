...
Upgrading from `1.x / v3` or `v4` to `2.0.1 / v5` requires upgrading every device. `v4` devices can keep the same 26-character pairing key; old six-digit-code settings are backed up before migration. Old and new protocols do not interoperate and never downgrade to plaintext.
